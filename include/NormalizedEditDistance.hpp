...
namespace AASS{
	namespace editdistance{
		...
		inline double GLS(const std::string& string_to_modify, const std::string& unchanged, std::string& out){
			...
			int x_size = string_to_modify.size();
			int y_size = unchanged.size();
			int dist = levenshteinDistance(string_to_modify, unchanged, out);
...
// 				std::cout << "Strings are equal" << std::endl;
				...
				std::string out_xx;
				std::string out_yy;
				int GLS_XX = GLS(string_to_modify, string_to_modify, out_xx);
// 				std::cout << "GLS_xx : " << GLS_XX << std::endl;
				int GLS_YY =  GLS(unchanged, unchanged, out_yy);
// 				std::cout << "GLS_yy : " << GLS_YY << std::endl;
				int GLS_XY = GLS(string_to_modify, unchanged, out);
